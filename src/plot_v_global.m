...
presets = ParameterPresets.by_name('V_Amp');

% MAKE COLORBAR IN ITS OWN FIGURE
fig = PlotTools.create_default_figure('V Amplitudes - Colorbar', [100 600 600 800]);
PlotTools.create_standalone_colorbar(presets.CRange, presets.CTicks);
PlotTools.save_image(fig, '/global_v_amp/global_v_amp_colorbar');

simulations = Simulation.list_all();
for plot_index = 1:1:length(simulations)
  sim = simulations(plot_index);
  fig = PlotTools.create_default_figure(['Global V Amplitudes - simulation #' sim.Number]);
  file = NetCDFFile.read_parameter_file(sim, presets);
  PlotTools.plot_parameter(presets, file);
  PlotTools.save_image(fig, ['/global_v_amp/global_v_amp_' sim.Number]);
end
