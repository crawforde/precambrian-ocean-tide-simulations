...
fig = PlotTools.create_default_figure('S Values - Colorbar', [100 600 600 800]);
PlotTools.create_standalone_colorbar(presets.CRange, presets.CTicks);
PlotTools.save_image(fig, '/global_s/global_s_colorbar');

simulations = Simulation.list_all();
for plot_index = 1:1:length(simulations)
  sim = simulations(plot_index);
  fig = PlotTools.create_default_figure(['Global S Values - simulation #' sim.Number]);
  file = NetCDFFile.read_parameter_file(sim, presets);
  PlotTools.plot_parameter(presets, file);
  PlotTools.save_image(fig, ['/global_s/global_s_' sim.Number]);
end
